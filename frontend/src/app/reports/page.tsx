...
const demoReport = `# Weekly Documentation Report
## VaultPay API Docs — Week of March 3, 2026

---

## Traffic Summary
- **Total Page Views**: 28,543 (+12% vs last week)
- **Unique Visitors**: 12,876 (+8%)
- **Avg Reading Time**: 2m 27s

## Top Pages
1. /docs/authentication — 4,821 views
2. /docs/payments/create — 3,654 views
3. /docs/webhooks — 3,201 views
4. /docs/errors — 2,987 views
5. /docs/getting-started — 2,654 views

## Search Analytics
- **Total Searches**: 1,538
- **Zero-Result Rate**: 26%
- Top failed queries: "payment retry logic", "rate limiting", "batch payments", "error code 4012"

## Coverage
- **Overall Score**: 78%
- 5 features fully documented, 2 partial, 1 undocumented
- **Action needed**: "Batch Payments" has no documentation

## Content Freshness
- 4 files updated this month (fresh)
- 3 files aging (31-90 days)
- 2 files stale (91-180 days)
- 1 file abandoned (>180 days): disputes.md

## Recommendations
1. Create documentation for Batch Payments endpoint
2. Update disputes.md (last touched 197 days ago)
3. Add content for "rate limiting" — high search volume, no results
4. Review 3D Secure docs (partial coverage)

---

*Generated by DocPulse on ${new Date().toLocaleDateString()}*`;
...
export default function ReportsPage() {
  ...
  const generateReport = async () => {
    setLoading(true);
    try {
      const text = await fetchAPI<string>("/reports/weekly");
      if (text === "demo_report") {
        setReport(demoReport);
      } else {
        setReport(typeof text === "string" ? text : JSON.stringify(text));
      }
    } catch {
      setReport(demoReport);
    } finally {
